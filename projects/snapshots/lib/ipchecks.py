...
def get_agg_v4(hostname):
    ...
    if error:
        print(error)
    ipv4_nei = (
        str(output, "utf-8").split("\n")[0].split()[3]
    )  # split string and select only the IPv4 address
...
class GetNeighborIPs(IPChecks):
    ...
    def get_ibgp_ips(self):
        ...
        if not self.circuit["ipv4_neighbor"] or not self.circuit["ipv6_neighbor"]:
            ...
            try:
                self.circuit["ipv6_neighbor"] = (
                    str(output, "utf-8").split("\n")[1].split()[4]
                )  # split string and select only the IPv6 address

            except:
                ipv6_nei = None
                if not self.circuit["ipv6_neighbor"]:
                    print(
                        f"\t\t\t* WARNING: No AAAA record for {self.hostname}.\n\t\t\tEnter manually if v6 Peering exists and re-run."
                    )
    ...
    def get_ebgp_static_ips(self, device_type, connection):
        ...
        if (
            not self.circuit["ipv4_neighbor"] or not self.circuit["ipv6_neighbor"]
        ) and self.circuit["port"]:

            if device_type == "junos":
                junos_parse = ParseData(device_type)

                arp = connection.rpc.get_arp_table_information(
                    interface=self.circuit["port"]
                )
                nd = connection.rpc.get_ipv6_nd_information(
                    interface=self.circuit["port"]
                )
                ...
                arp = connection.get(
                    (filters.arp.format(interface=self.circuit["port"]))
                )
