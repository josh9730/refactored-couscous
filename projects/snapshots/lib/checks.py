...
class CircuitChecks(Device):
    ...
    def get_circuit_main(self):
        ...
        for self.circuit in self.circuits_dict:
            ...
            if self.circuits_dict[self.circuit]["service"] == "static":
                ...
                print(f"\t\t1) IPv4 Static")
                ipv4_static = self.get_circuit_static(
                    "inet.0", self.circuits_dict[self.circuit]["ipv4_routes"]
                )

                try:
                    print(f"\t\t2) IPv6 Static")
                    ipv6_static = self.get_circuit_static(
                        "inet6.0", self.circuits_dict[self.circuit]["ipv6_routes"]
                    )
                ...
                self.addresses = [
                    self.circuits_dict[self.circuit]["ipv4_neighbor"],
                    self.circuits_dict[self.circuit]["ipv6_neighbor"],
                ]
    ...
    def adv_counts(self, adv_counts):
        ...
        for index, adv_count in enumerate(adv_counts):
            ...
            if not adv_count:
                if index == 0:
                    self.ipv4_adv_count = "No Adv. Routes"
                elif index == 1:
                    self.ipv6_adv_count = "No Adv. Routes"
            ...
            # HPR Full Table ~ 20,000 / 20,000
                ...
                if adv_count > 20000:
                    if index == 0:
                        self.ipv4_adv_count = "Full HPR IPv4 Table"
                    elif index == 1:
                        self.ipv6_adv_count = "Full HPR IPv6 Table"

                elif adv_count > 0:
                    if index == 0:
                        self.ipv4_adv_count = adv_count
                    elif index == 1:
                        self.ipv6_adv_count = adv_count
            ...
            # DC Full Table ~ 800,000 / 100,000
                ...
                if adv_count > 800000 and index == 0:
                    self.ipv4_adv_count = "Full DC IPv4 Table"
                elif adv_count > 100000 and index == 1:
                    self.ipv6_adv_count = "Full DC IPv6 Table"
    ...
    def get_circuit_bgp_xr(self):
        ...
        if self.vrf == "hpr":
            ...
            self.ipv4_circuit_data, self.ipv6_circuit_data, self.start_time, = NapalmXR(
                elapsed_time, self.username, self.device_name, addresses=self.addresses
            ).get_circuit_hpr()
    ...
    def circuit_bgp_xr(self, address):
        ...
        circuit_raw = self.connection.get(
            (filters.bgp_routes_filter.format(version=version, neighbor=address))
        )
        default_raw = self.connection.get(
            (filters.bgp_default_filter.format(version=version, neighbor=address))
        )
    ...
    def get_circuit_iface_xr(self):
        ...
        iface_name_raw = self.connection.get(
            (filters.iface_name_circuit.format(iface=self.port))
        )
    ...
    def circuit_bgp_junos(self, address):
        ...
        counts_raw = self.connection.rpc.get_bgp_neighbor_information(
            neighbor_address=address
        )
        adv_count, rx_count, self.vrf = junos_parse.parse_circuit_bgp_nei_junos(
            counts_raw
        )
        ...
        circuit_raw = self.connection.rpc.get_route_information(
            brief=True, table=table, peer=address, receive_protocol_name="bgp"
        )
        ...
        default_raw = self.connection.rpc.get_route_information(
            destination=default,
            exact=True,
            neighbor=address,
    ...
    def rx_routes_junos(
        self, routes_list, table, address, connection_static="", static=False
    ):
        ...
        for route in routes_list:
            ...
            if static:
                route_data = connection.rpc.get_route_information(
                    destination=route,
                    table=table,
                    exact=True,
                    detail=True,
    ...
    def get_circuit_iface_junos(self):
        ...
        optics_raw = self.connection.rpc.get_interface_optics_diagnostics_information(
            interface_name=self.port
        )
        junos_parse.parse_circuit_optics_junos(optics_raw)
        iface_raw = self.connection.rpc.get_interface_information(
            interface_name=self.port, extensive=True
        )
        self.iface = junos_parse.parse_circuit_iface_junos(iface_raw)

    def get_circuit_isis_junos(self):

        isis_raw = self.connection.rpc.get_isis_interface_information(
            interface_name=self.port, extensive=True
        )
        junos_parse = ParseData(self.device_type)
    ...
    def get_circuit_static(self, table, routes_list):
        ...
        rx_routes = self.rx_routes_junos(
            routes_list, table, address, connection_static=connection, static=True
        )
...
class DeviceChecks(Device):
    ...
    def napalm_device_xr(self):
        ...
        elapsed_time = int(time.time() - self.start_time)
        self.software, self.msdp, self.ifaces, self.start_time = NapalmXR(
            elapsed_time, self.username, self.device_name, circuit=False
        ).get_device_xr()
    ...
    def get_device_junos(self):
        ...
        iface_raw = self.connection.rpc.get_interface_information(
            detail=True, statistics=True
        )
        iface = junos_parse.device_iface_junos(iface_raw)

        print(f"\t5) Software")
        software = etree_to_dict(self.connection.rpc.get_software_information())[
        ...
        if vrf == "hpr":
            ...
            isis_hpr_raw = self.connection.rpc.get_isis_adjacency_information(
                instance="hpr"
            )
            ...
            pim_hpr_raw = self.connection.rpc.get_pim_neighbors_information(
                instance="hpr"
            )
