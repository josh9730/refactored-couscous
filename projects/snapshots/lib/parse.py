    ...
    if children:
        ...
        for dc in map(etree_to_dict, children):
            for k, v in dc.items():
                dd[k].append(v)
        etree_dict = {
            etree_data.tag: {k: v[0] if len(v) == 1 else v for k, v in dd.items()}
        }
    if etree_data.attrib:
        etree_dict[etree_data.tag].update(
            ("@" + k, v) for k, v in etree_data.attrib.items()
        )
    if etree_data.text:
...
class ParseData:
    ...
    def parse_circuit_arp_junos(self, data):
        ...
        try:
            ipv4 = data["arp-table-information"]["arp-table-entry"]["ip-address"]
        except:
            ipv4 = data["arp-table-information"]["arp-table-entry"][0]["ip-address"]
    ...
    def parse_circuit_nd_junos(self, data):
        ...
        try:
            ipv6 = data["ipv6-nd-information"]["ipv6-nd-entry"][0][
    ...
    def parse_circuit_bgp_nei_junos(self, data):
        ...
        if type(full_path) == list:
            ...
            full_path["name"] == "inet.0" or "inet6.0" or "hpr.inet.0" or "hpr.inet6.0"
        ):
    ...
    def parse_circuit_bgp_junos(self, data):
        ...
        try:
            med = full_path["rt-entry"]["metric"]
        except:
            med = "Not set"

        lp = full_path["rt-entry"]["local-preference"]
        as_path = full_path["rt-entry"]["bgp-path-attributes"][
            "attr-as-path-effective"
        ]["attr-value"].rstrip(" I")
        community = full_path["rt-entry"]["communities"]["community"]
        next_hop = full_path["rt-entry"]["nh"]["to"]

        route_dict = self.create_routes_dict(
            route, next_hop, lp, as_path, med, community
        )
    ...
    def parse_circuit_default_junos(self, data):
        ...
        try:
            if (
                data["route-information"]["route-table"]["rt"]["rt-destination"]
                == "0.0.0.0/0"
                or "::/0"
            ):
                default = "Yes"
    ...
    def parse_circuit_optics_junos(self, data):
        ...
        try:
            self.optics_dict = {}
            full_path = data["interface-information"]["physical-interface"][
                "optics-diagnostics"
    ...
    def parse_circuit_isis_junos(self, data):
        ...
        try:
            ...
            if full_path["interface-level-data"]["adjacency-count"] == "1":
                state = "Up"
            else:
                state = "Down"

            try:
                v6_metric = full_path["interface-level-data"][
                    "isis-interface-level-topology"
                ]["isis-topology-metric"]
            except:
                v6_metric = "No IPv6 Adjacency"
    ...
    def parse_circuit_arp_xr(self, data):
        ...
        try:
            for ip in data["rpc-reply"]["data"]["arp"]["nodes"]["node"]["entries"][
                "entry"
            ]:
    ...
    def parse_circuit_nd_xr(self, data):
        ...
        try:
            for ip in data["rpc-reply"]["data"]["ipv6-node-discovery"]["nodes"]["node"][
                "neighbor-interfaces"
            ]["neighbor-interface"]["host-addresses"]["host-address"]:
                if ip["is-router"] == "true" and not ip["host-address"].startswith(
                    "fe80"
                ):
    ...
    def parse_circuit_bgp_xr(self, data):
        ...
        try:
            for table in data["rpc-reply"]["data"]["oc-bgp"]["bgp-rib"][
                "afi-safi-table"
            ].keys():
                version = re.match("(ipv(4|6))-unicast", table).groups()[0]

            full_path = data["rpc-reply"]["data"]["oc-bgp"]["bgp-rib"][
                "afi-safi-table"
            ][f"{version}-unicast"]["open-config-neighbors"]["open-config-neighbor"]
            ...
            try:
                ...
                if type(full_path["adj-rib-in-post"]["routes"]["route"]) == list:
                    for route in full_path["adj-rib-in-post"]["routes"]["route"]:
                        ...
                        nh, lp, asp, med, comm = self.parse_circuit_bgp_xr_routes(
                            path, version
                        )
                        rx_routes.update(
                            self.create_routes_dict(
                                route["route"], nh, lp, asp, med, comm
                            )
                        )
                    ...
                    nh, lp, asp, med, comm = self.parse_circuit_bgp_xr_routes(
                        route["route-attr-list"], version
                    )
                    rx_routes.update(
                        self.create_routes_dict(route["route"], nh, lp, asp, med, comm)
                    )
    ...
    def parse_circuit_default_xr(self, data):
        ...
        try:
            default = data["rpc-reply"]["data"]["bgp"]["instances"]["instance"][
                "instance-active"
            ]["default-vrf"]["afs"]["af"]["neighbor-af-table"]["neighbor"]["af-data"][
                "is-default-originate-sent"
            ]
    ...
    def parse_circuit_bgp_xr_routes(self, path, version):
        ...
        comm = []
        for community in path["community"]:
            comm.append(community["objects"])
    ...
    def parse_circuit_iface_name_xr(self, data):
        ...
        try:
            self.iface_name = data["rpc-reply"]["data"]["interface-configurations"][
    ...
    def parse_circuit_iface_xr(self, data):
        ...
        full_path = data["rpc-reply"]["data"]["infra-statistics"]["interfaces"][
            "interface"
        ]
        ...
        iface_dict = {
            full_path["interface-name"]: {
                ...
                "Stats": {
                    "Errors": {
                        "Rx Errors": full_path["interfaces-mib-counters"][
                            "input-errors"
                        ],
                        "Tx Errors": full_path["interfaces-mib-counters"][
                            "output-errors"
                        ],
                    },
                    "Drops": {
                        "Rx Drops": full_path["interfaces-mib-counters"]["input-drops"],
                        "Tx Drops": full_path["interfaces-mib-counters"][
                            "output-drops"
                        ],
                    },
                },
    ...
    def parse_circuit_isis_xr(self, data):
        ...
        try:
            ...
            try:
                v4_metric = full_path["interface-status-and-data"]["enabled"][
                    "per-topology-data"
                ][0]["status"]["enabled"]["level2-metric"]
            except:
                v4_metric = full_path["interface-status-and-data"]["enabled"][
                    "per-topology-data"
                ]["status"]["enabled"]["level2-metric"]
            try:
                v6_metric = full_path["interface-status-and-data"]["enabled"][
                    "per-topology-data"
                ][1]["status"]["enabled"]["level2-metric"]
            except:
                v6_metric = "No IPv6 Adjacency"

            if (
                full_path["interface-status-and-data"]["enabled"]["clns-data"][
                    "clns-status"
                ]["status"]
                == "isis-up"
            ):
    ...
    def device_optics_junos(self, data):
        ...
        for port in data["interface-information"]["physical-interface"]:
            ...
            all_lane_dict = {}
            if (
                type(port["optics-diagnostics"]["optics-diagnostics-lane-values"])
                == list
            ):
                for lane in port["optics-diagnostics"][
                    "optics-diagnostics-lane-values"
                ]:
                    lane_num = lane["lane-index"]
                    lane_dict = {
                ...
                lane_num = port["optics-diagnostics"]["optics-diagnostics-lane-values"][
                    "lane-index"
                ]
                all_lane_dict = {
                    f"Lane {lane_num}": {
                        "Rx Power": port["optics-diagnostics"][
                            "optics-diagnostics-lane-values"
                        ]["laser-rx-optical-power-dbm"]
                        + "dBm",
                        "Tx Power": port["optics-diagnostics"][
                            "optics-diagnostics-lane-values"
                        ]["laser-output-power-dbm"]
                        + "dBm",
                    }
                }
            optic_dict = {port["name"]: all_lane_dict}
    ...
    def device_iface_junos(self, data):
        ...
        for port in data["interface-information"]["physical-interface"]:
            ...
            if port["oper-status"] == "up" and port["name"].startswith(
                ("xe", "et", "ge")
            ):
                port_dict = {
                    port["name"]: {
    ...
    def device_isis_xr(self, data):
        ...
        if isinstance(full_path, dict):
            ...
            for neighbor in full_path["neighbors"]["neighbor"]:
                if neighbor["neighbor-state"] == "isis-adj-up-state":
                    state = "Up"
                else:
                    state = "Down"
    ...
    def device_pim_xr(self, data):
        ...
        pim_dict = {}
        for neighbor in data["rpc-reply"]["data"]["pim"]["active"]["default-context"][
            "neighbor-old-formats"
        ]["neighbor-old-format"]:
            nei_dict = {
                neighbor["interface-name"]: {"Neighbor": neighbor["neighbor-address"]}
            }
            pim_dict.update(nei_dict)
    ...
    def device_stats_xr(self, data):
        ...
        for iface in data["rpc-reply"]["data"]["infra-statistics"]["interfaces"][
            "interface"
        ]:
            try:
                iface_dict = {
    ...
    def device_iface_xr(self, data):
        ...
        iface_dict = {}
        for iface in data["rpc-reply"]["data"]["ethernet-interface"]["interfaces"][
            "interface"
        ]:
            try:
    ...
    def device_bgp_xr(self, data):
        ...
        bgp_dict = {}
        for nei_def_vrf in data["rpc-reply"]["data"]["bgp"]["instances"]["instance"][
            "instance-active"
        ]["default-vrf"]["afs"]["af"]:
            ...
            for nei in nei_def_vrf["neighbor-af-table"]["neighbor"]:
                ...
                try:
                    desc = nei["description"]
                except:
                    desc = "None"
                try:
                    pref = nei["af-data"][0]["prefixes-accepted"]
                except:
                    pref = nei["af-data"]["prefixes-accepted"]
        ...
        try:
            for nei_hpr in data["rpc-reply"]["data"]["bgp"]["instances"]["instance"][
                "instance-active"
            ]["vrfs"]["vrf"]["neighbors"]["neighbor"]:

                try:
                    desc = nei_hpr["description"]
                except:
                    desc = "None"
                try:
                    pref = nei_hpr["af-data"][0]["prefixes-accepted"]
                except:
                    pref = nei_hpr["af-data"]["prefixes-accepted"]

                nei_dict = {
                    nei_hpr["neighbor-address"]: {
                        "Description": desc,
                        "Peer AS": nei_hpr["remote-as"],
                        "State": nei_hpr["connection-state"]
                        .lstrip("bgp-st-")
                        .capitalize(),
                        "Accepted Prefixes": pref,
                    }
